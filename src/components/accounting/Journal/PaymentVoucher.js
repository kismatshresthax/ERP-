...
export default function PaymentVoucher(props) {
  const userSessionContext = React.useContext(UserSessionContext);
  ...
  const [journalname, setjournalname] = useState("Payment Voucher");
  ...
  // const uid = () => {
  //   const h=new Date.toString(36);
  //   console.log(h)
  //   const i=Math.floor(100000 + Math.random() * 900000).toString(36).substring(2);
  //   return h+i;
  // };
  //  console.log(uid)
  ...
  useEffect(async() => {
   await axios
      .get(
        `${config.APP_CONFIG}/Account/LedgerReport/api/${accountingDate}/${companyId}`,
        {
          headers: { Authorization: userSessionContext.token },
        }
      )
      .then((res) => {
        if (res.data.status_code === 401) {
          ...
          var trans =
            "PAV" + companyFiscalyear + "/" + Date1 + "/" + res.data.msg;
   ...
   await axios
      ...
      .then((res) => {
        if (res.data.status_code === 401) {
          ...
          // console.log(res.data.msg);
          let vendorList = res.data.msg.map((item) => ({
            value: item.id,
            label: item.firstName,
          }));

          //vendorList = [{ id: 0, title: 'Select' }].concat(vendorList);
  ...
  const Addjournal = () => {
    ...
    if (dr_total !== cr_total) {
      ...
      let journal_entry_transaction = {
        ...
        reference: "Payment made to " + vendor.label,
        narration: remarks,
  ...
  return (
    <div
      className="content-wrapper iframe-mode paymentVoucher"
      data-widget="iframe"
              ...
              <div className="row g-6" style={{ padding: "5px 0 15px 0" }}>
                <div className="col-md-3 partyName">
                  ...
                  {/* <select
                      ...
                      name="vendor"
                      id="vendor"
                       style={{ paddingTop: ".33rem", paddingBottom: ".33rem" }}
                      value={vendorList.label}
                      onClick={(e) => {
  ...
  <CardHeader
      ...
      <Card variant="outlined">
        ...
        <CardContent>
          <div class="input-group journal-tab5">
            <div class="input-group-prepend">
