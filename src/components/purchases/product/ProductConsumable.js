...
export default function ProductConsumable(props) {
  ...
  const load_table = async() => {
   await axios
      .get(`${config.APP_CONFIG}/Products/productbyType/api/isConsumable`, {
        headers: { Authorization: userSessionContext.token },
      })
      .then((res) => {
        if (res.data.status_code === 200) {
    ...
    <div>
      ...
      >
        ...
        <Toolbar>
          ...
          />
            <SelectFilter   className={classes.searchInput} options={searchrecord} placeholder="Category"/>
