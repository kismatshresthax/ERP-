...
export default function SalesSummaryTablePage(props) {
  ...
  const [isvoidPopup, setIsvoidPopup] = useState();
 ...
 let company_data={
  ...
  const handleExport = () => {
    ...
    const dataInfo = {
      ...
      titleCell: "A4",
      titleRange: "A4:L4",
      tbodyRange: `A5:M${finalData.length}`,
      theadRange:
    ...
    <div>
      <div>
        ...
        {/* {setIsvoidPopup? (
          <Popup
            title="Sales void"
            openPopup={isvoidPopup}
            setPopups={setIsvoidPopup}
            // setPopups={() => {
            //   setIsNewPopup(false);
            // }}
          >
            <SalesVoid 
              datas={records.filter((x) => x.id === isvoidPopup)[0] || null}
            />
          </Popup>
        ) : null} */}
...
{ isvoidPopup?( <SalesVoid   datas={records.filter((x) => x.id === isvoidPopup)[0] || null}
/>):null}
                ...
                <TableBody>
                  {recordsAfterPagingAndSorting().map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                      {(item.dateOfSales.slice(0,16))+ " " + NepaliDate(item.dateOfSales)}
                      </TableCell>
                      ...
                      <TableCell>
                        ...
                        >
                         ...
                         <Controls.ActionButton
                          ...
                          // setIsvoidPopup(item.id);
                      
                            history.push(`/sales/salesvoid/${item.id}`);
...
const PopupHandler = (props) => {
  ...
  useEffect(async() => {
    if (props.isEditPopup) {
     await axios
        .get(
          `${config.APP_CONFIG}/Sales/SalesSummary/api/byCompany/${props.companyId}/${props.isEditPopup}`,
          {
            headers: { Authorization: userSessionContext.token },
          }
        )
...
//       {datas.id === undefined ? step_mapper[0] : step_mapper[step]}
//     </div>
//   );
// };
