        ...
        <TableBody>
          {
            data.length > 0 && data.map((customer,idx) => {
              ...
              return (
                <TableRow key={customer.idx}>
                  <TableCell>{customer.customer_name}</TableCell>
                  <TableCell align='right'>{parseInt(customer.Amount_1)+parseInt(customer.Amount_2)}</TableCell>
                  <TableCell align='right'>{parseFloat(Math.floor(((new Date())-(new Date(customer.bill_1_date)))/(1000*60*60*24)))<30 || parseFloat(Math.floor(((new Date())-(new Date(customer.bill_2_date)))/(1000*60*60*24)))<30 ? <p>{customer.Amount_1 + customer.Amount_2}</p>: <p>-</p>}</TableCell>
